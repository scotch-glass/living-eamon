...
5. **`KARMA_SYSTEM.md`** — single source of truth for the karma system design (PICSSI virtues + attributes + consumables (HP / mana / stamina+fatiguePool) + atoms + NPC affection + flags). All numerical values for the karma system live here. As of 2026-04-29 evening, all open design questions are resolved and §6 Approval Gate awaits "begin Sprint 1" sign-off. **2026-05-06 update at the top of the file** introduces three locked rules: (1) Passion-vs-Spirituality combat trade-off (physical combat `+P / -Sp`, casting `+Sp / -P`, applies to choice of action in combat only — does NOT extrapolate to non-combat spigots like prayer or tavern visits); (2) Courage robust paths — for a module to be **balanced** it should include at least one "great odds" encounter, plus a wider set of secondary Courage spigots (duels, feared-zone-solo, refusing supernatural flight); intentionally-unbalanced modules can skip it; (3) Integrity quest-reward tier table (short `+3` / medium `+5` / campaign-defining `+10`).
6. **`MODULE_SYSTEM.md`** — Stage I draft (awaits approval before implementation) for the Ink + GPE module-authoring system. Specifies Inkle's Ink narrative scripting, the Growth Path Equalizer (GPE) authoring CLI for per-virtue balance scoring, a stable EXTERNAL function contract that mutates PICSSI / NPC affection / flags / HP / combat triggers, and combat-PICSSI feedback hook points. References KARMA_SYSTEM.md as source of truth — never duplicates values.
7. **`KARMA_IMPLEMENTATION_PLAN.md`** — sprint-by-sprint wiring plan (Sprints 0–7) with file-, function-, and DDL-level detail. References KARMA_SYSTEM.md as source of truth — never duplicates values. Load this when implementing karma; load KARMA_SYSTEM.md when designing.
8. **`Public_Domain_Rules.md`** is now the single consolidated PD-safety doc (as of April 19, 2026). Authoritative Safe Harbor / Radioactive lookup at the top of **`GAME_DESIGN.md`**.
9. **`lore/hyborian-pd/MODULE_PLAN.md`** — methodology for converting PD stories into adventure modules.
10. **`lore/scrolls-of-thoth/INDEX.md`** + the 15 scroll files — the in-game Scrolls of Thoth, Kybalion-derived (Hermes→Thoth substitution), each with riddle frontmatter for the read-verification gate (KARMA_SYSTEM.md §2.10 + project memory `project_scroll_riddle_verification.md`).
11. `.claude/projects/-Users-joshuamcclure-Desktop-living-eamon/memory/MEMORY.md` — memory index.
12. §5.4 below — body-zone combat system (all 6 phases complete; body-zone also serves as the model for the stamina+fatiguePool system per KARMA_SYSTEM.md §2.3).
13. §5.10–§5.16 below — recent additions: mana system, merchant purchases, bleed/poison, charity-barrel Honor (legacy — being cold-deleted in Sprint 2), sidebar tabs, item action menu, apply-poison-to-weapon, expertise→maxMana rename.
...
§ Database transition (deferred — 2026-04-30)

The project will eventually migrate quest definitions, NPC↔quest associations, dialogue branches, and atom definitions from TypeScript registries into Supabase tables (shadow first, with TS staying canonical at runtime). **This migration is explicitly deferred until Sprint 8 (8f / 8g / 8h) ships AND KARMA Sprint 7 (Sorcery / Outer Dark / Circles 5–8) lands.** Until then, the in-memory TS registries (`lib/quests/engine.ts:QUEST_REGISTRY`, `lib/quests/dialogue.ts:REGISTRY`, etc.) remain the only source of truth, and the empty `quest_definitions` table from migration `20260430110000_quest_engine_bedrock.sql` stays empty.

Visibility into the current registry lives in derived form at [docs/quest-registry.md](./docs/quest-registry.md) (regenerated by `npm run registry:dump`). The neuro-emotional reading of the existing AffectVector axes is documented at [docs/affect-axes.md](./docs/affect-axes.md). Both surfaces are designed to scale into DB-backed views once the transition begins.

For the full deferred-DB plan rationale, see `~/.claude/plans/good-questions-ultimately-the-radiant-cat.md`. For the original game-architecture stance ("game data in code; player state + caches in DB"), see CLAUDE_CONTEXT.md §4 below.
