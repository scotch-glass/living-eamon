...
0.5. **`docs/doc-graph.md`** — auto-generated adjacency view of every doc + every Edge Vector. Loading this gives you the full topology in one read instead of grep-and-load. Stats, most-connected nodes, per-doc outgoing edges, open questions. Regenerated by `npm run graph:build`.
0.6. **`docs/launch-readiness.md`** — auto-generated prioritized list of "what to authorize next to launch." Reads `LAUNCH_CRITERIA.md` + `docs/doc-graph.json` and surfaces the critical-path bottleneck. Regenerated by `npm run launch:status`.
