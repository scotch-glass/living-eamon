...
//      admin review UI).
//
// Status after generate is always "pending" — an admin has to
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (text.length > 4000) {
    return NextResponse.json(
      { ok: false, error: `text length ${text.length} exceeds 4000-char limit` },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const meta = await uploadVersion(audioId, audio, contentType, text, body.instructions);
